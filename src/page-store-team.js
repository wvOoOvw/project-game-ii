...
class PageStore {
  constructor() {
    ...
    this.InstanceCard
    this.InstanceTeam
    this.InstancePreview = this.instancePreview()
    this.InstanceBanner = this.instanceBanner()

    this.instance()
  }

  get bannerHeight() {
    return (windowHeight - 60 - safeTop) / Imitation.state.info.team.length
  }
  ...
  instanceScroll() {
    ...
    this.InstanceScroll.scrollY = this.bannerHeight + this.cardHeight + this.teamHeight - this.emptyHeight - this.InstanceScroll.height + 24
  }
  ...
  instanceTeam() {
    this.InstanceTeam = this.team.map((card, index) => {
      ...
      option.y = 84 + parseInt(index / 4) * (option.height + 6) + this.bannerHeight + safeTop
  ...
  instanceCard() {
    this.InstanceCard = this.card.map((card, index) => {
      ...
      option.y = 96 + parseInt(index / 4) * (option.height + 12) + this.bannerHeight + this.teamHeight - this.emptyHeight + safeTop
  ...
  drawBanner(offsetY) {
    this.InstanceBanner.offsetY = 0 - offsetY

    this.InstanceBanner.render()
  }

  drawTeam(offsetY) {
    if (this.team.length === 0) return

    const option = { x: 12, y: 72 + safeTop + this.bannerHeight - offsetY, width: windowWidth - 24, height: this.teamHeight, radius: 12 }
  ...
  drawCard(offsetY) {
    const option = { x: 12, y: 84 + this.bannerHeight + this.teamHeight - this.emptyHeight + safeTop - offsetY, width: windowWidth - 24, height: this.cardHeight, radius: 12 }
